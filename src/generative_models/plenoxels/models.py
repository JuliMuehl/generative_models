...
class DifferentiableRenderer(torch.nn.Module, ABC):
    def __init__(self, bg_rgb = [1.0, 1.0, 1.0]):
        super().__init__()
        self.register_buffer("bg_rgb", torch.tensor(bg_rgb, dtype=torch.float))

    @abstractmethod
    def sample(self, xzy, dirs):
        pass
...
class SH2VoxelGrid(DifferentiableRenderer):
