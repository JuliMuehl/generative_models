    ...
    void main(){
        ...
        for(int i = 0;i<num_samples;i++){
            ...
            if(cond.x >= 0 && cond.y >= 0.0 && cond.z >= 0.0){
                float density = texture(u_density_grid, xtex).x;
                float alpha = 1.0 - exp(-density*delta);
                vec3 color = texture(u_color_grid, xtex).xyz;
...
class SH2VoxelRenderer:
    ...
    void main(){
        ...
        for(int i = 0;i<num_samples;i++){
            ...
            if(cond.x >= 0 && cond.y >= 0.0 && cond.z >= 0.0){
                float density = texture(u_density_grid, xtex).x;
                float alpha = 1.0 - exp(-density*delta);
                vec3 color = eval_sh2_color(xtex, ray_direction);
                radiance += alpha * transmittance * color;
                transmittance *= (1.0 - alpha);
            }else{
                //Since the box is convex we can stop once we leave it
    ...
    def load_grid(self):
        if self.grid_mtime is None or os.path.getmtime(self.grid_file) > self.grid_mtime:
            with np.load(self.grid_file) as voxel_data:
                print("RELOADING")
                self.sh2_C, sh2_grid, density_grid = voxel_data["sh2_C"], voxel_data["sh2_grid"], voxel_data["density_grid"]
