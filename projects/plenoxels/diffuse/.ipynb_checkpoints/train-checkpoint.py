...
from model import DiffuseVoxelGrid

if __name__ == "__main__":
    device = "xpu" if torch.xpu.is_available() else "cpu"
    model = DiffuseVoxelGrid(scale=1, N=128, tv_loss_weight=1e-4, sparsity_loss_weight=1e-7).to(device)
    image_dir = "images"
    ...
    optimizer = torch.optim.AdamW([{'params':model.color_grid, 'lr' : 5}, {'params':model.density_grid, 'lr':5}])
    n_splits = 256
    epochs, batch_size = 16000, len(colors.view(-1,3)) // n_splits
    pbar = tqdm(range(epochs))
    scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=1 ** (1.0 / epochs))
    for epoch in pbar:
        ...
        if epoch % 100 == 0:
            img = (model(origins[0].view(-1, 3), dirs[0].view(-1, 3)).view(512, 512, 3) * 255).to(torch.uint8)
            Image.fromarray(img.detach().to("cpu").numpy()).save("validation.png")
            torch.save(model, "diffoxels.pt")
