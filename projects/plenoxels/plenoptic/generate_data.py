from generative_models.plenoxels.renderers import SpecularSceneRenderer 
...
def main():    
    renderer = SpecularSceneRenderer()
    train_positions = [unit_sphere(theta,phi) for theta in np.linspace(np.pi/8, np.pi/2 - 0.1, 16) for phi in np.linspace(0, 2 * np.pi, 16)]
